...
public class LikeCountPlayListFinder implements IPlayListFinder{
    private final boolean m_isDesc;

    public LikeCountPlayListFinder(boolean isDesc){
    ...
    public AdminPlayListSearchResultDTO getPlayList(int pageNum, int pageCount, int[] licenseIds, int[] subjectIds, IAdminRepository repository) {
        List<AdminYoutubePlayListDTO> pl = repository.findPlayListByLike(m_isDesc, licenseIds, subjectIds, pageNum, pageCount);
